...
def compare_models(y, tx, gammas, lambdas, k_fold, seed=101):
    ...
    for k in range(k_fold):
        ...
        w, _ = ridge_regression(y_train, x_train, lambdas[1])
        y_test_pred = predict_labels(w, x_test)
        acc_test = np.sum(y_test_pred==y_test) / y_test_pred.shape[0]
        acc_ridge.append(acc_test)

    return np.mean(acc_logreg, axis=0), np.mean(acc_logreg_reg, axis=0), np.mean(acc_ls_GD, axis=0), np.mean(acc_ls_SGD, axis=0), np.mean(acc_ridge, axis=0)